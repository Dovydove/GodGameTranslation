...
class PathInterpolatorApi14 implements Interpolator {
    ...
    PathInterpolatorApi14(Path path) {
        ...
        this.f18mX = new float[numPoints];
        this.f19mY = new float[numPoints];
        float[] position = new float[2];
        for (int i = 0; i < numPoints; i++) {
            pathMeasure.getPosTan((((float) i) * pathLength) / ((float) (numPoints - 1)), position, null);
            this.f18mX[i] = position[0];
            this.f19mY[i] = position[1];
        }
    }
    ...
    public float getInterpolation(float t) {
        ...
        int startIndex = 0;
        int endIndex = this.f18mX.length - 1;
        while (endIndex - startIndex > 1) {
            int midIndex = (startIndex + endIndex) / 2;
            if (t < this.f18mX[midIndex]) {
                endIndex = midIndex;
            } else {
                startIndex = midIndex;
            }
        }
        float xRange = this.f18mX[endIndex] - this.f18mX[startIndex];
        if (xRange == 0.0f) {
            return this.f19mY[startIndex];
        }
        float fraction = (t - this.f18mX[startIndex]) / xRange;
        float startY = this.f19mY[startIndex];
        return ((this.f19mY[endIndex] - startY) * fraction) + startY;
    }
