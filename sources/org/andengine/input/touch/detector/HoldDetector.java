...
public class HoldDetector extends BaseDetector {
    ...
    public HoldDetector(IHoldDetectorListener pHoldDetectorListener) {
        this(TRIGGER_HOLD_MINIMUM_MILLISECONDS_DEFAULT, TRIGGER_HOLD_MAXIMUM_DISTANCE_DEFAULT, pHoldDetectorListener);
    }
    ...
    public boolean onManagedTouchEvent(TouchEvent pSceneTouchEvent) {
        ...
        switch (pSceneTouchEvent.getAction()) {
            ...
            case 2:
                ...
                this.mHoldX = pSceneTouchEvent.getX();
                this.mHoldY = pSceneTouchEvent.getY();
                long holdTimeMilliseconds2 = System.currentTimeMillis() - this.mDownTimeMilliseconds;
                if (holdTimeMilliseconds2 >= this.mTriggerHoldMinimumMilliseconds) {
                    if (this.mTriggering) {
                        triggerOnHold(holdTimeMilliseconds2);
                    } else {
                        float triggerHoldMaximumDistance2 = this.mTriggerHoldMaximumDistance;
                        if (this.mMaximumDistanceExceeded || Math.abs(this.mDownX - motionEvent.getX()) > triggerHoldMaximumDistance2 || Math.abs(this.mDownY - motionEvent.getY()) > triggerHoldMaximumDistance2) {
                            z = true;
                        }
                        this.mMaximumDistanceExceeded = z;
                        if (!this.mMaximumDistanceExceeded) {
    ...
    public void prepareHold(TouchEvent pSceneTouchEvent) {
        MotionEvent motionEvent = pSceneTouchEvent.getMotionEvent();
    ...
    /* access modifiers changed from: protected */
    public void triggerOnHold(long pHoldTimeMilliseconds) {
        if (this.mPointerID != -1) {
    ...
    /* access modifiers changed from: protected */
    public void triggerOnHoldFinished(long pHoldTimeMilliseconds) {
        this.mTriggering = false;
        if (this.mPointerID != -1) {
