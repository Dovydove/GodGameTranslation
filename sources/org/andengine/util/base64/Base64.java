...
public class Base64 {
    ...
    public static byte[] decode(byte[] input, int offset, int len, int flags) {
        ...
        } else if (decoder.f1226op == decoder.output.length) {
            return decoder.output;
        } else {
            byte[] temp = new byte[decoder.f1226op];
            System.arraycopy(decoder.output, 0, temp, 0, decoder.f1226op);
            return temp;
        }
    }
    ...
    public static byte[] encode(byte[] input, int offset, int len, int flags) {
        ...
        if ($assertionsDisabled || encoder.f1226op == output_len) {
