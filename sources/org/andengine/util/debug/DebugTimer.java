...
public class DebugTimer {
    ...
    public static final int INDENT_SPACES = SPLIT_STRING.length();
    private static final String SPLIT_STRING = "  Split: ";
    /* access modifiers changed from: private */
    public final DebugLevel mDebugLevel;
    ...
    public class DebugTime {
        ...
        public DebugTime(DebugTimer this$02, long pStartTime, String pLabel) {
        ...
        public void dump(int pIndent, String pPostfix) {
            ...
            char[] indent2 = new char[(DebugTimer.INDENT_SPACES * pIndent)];
            Arrays.fill(indent2, ' ');
            if (this.mChildren == null) {
                Debug.log(DebugTimer.this.mDebugLevel, new String(indent2) + "'" + this.mLabel + "' @( " + (this.mEndTime - this.mStartTime) + "ms )" + pPostfix);
                return;
            }
            ArrayList<DebugTime> children = this.mChildren;
            int childCount = children.size();
            Debug.log(DebugTimer.this.mDebugLevel, new String(indent2) + "'" + this.mLabel + "' {");
            for (int i = 0; i < childCount - 1; i++) {
                ((DebugTime) children.get(i)).dump(pIndent + 1, ",");
            }
            ((DebugTime) children.get(childCount - 1)).dump(pIndent + 1);
            Debug.log(DebugTimer.this.mDebugLevel, new String(indent2) + "}@( " + (this.mEndTime - this.mStartTime) + "ms )" + pPostfix);
        }
    ...
    public void dump(boolean pClear) {
        long now = System.currentTimeMillis();
        if (this.mDebugTimes.size() > 1) {
            Debug.m1284w(getClass().getSimpleName() + " not all ended!");
        }
