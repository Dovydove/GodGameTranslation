...
import org.andengine.util.adt.p029io.out.ByteBufferOutputStream;
...
public abstract class PVRTexture extends Texture {
    ...
    public enum PVRTextureFormat {
        ...
        public static PVRTextureFormat fromID(int pID) {
            PVRTextureFormat[] pvrTextureFormats;
    ...
    public static class PVRTextureHeader {
        ...
        static final byte[] MAGIC_IDENTIFIER = {80, 86, 82, 33};
    ...
    /* access modifiers changed from: protected */
    public void writeTextureToHardware(GLState pGLState) throws IOException {
        ...
        while (currentPixelDataOffset < dataLength) {
            if (currentLevel > 0 && !(width == height && MathUtils.nextPowerOfTwo(width) == width)) {
                Debug.m1284w("Mipmap level '" + currentLevel + "' is not squared. Width: '" + width + "', height: '" + height + "'. Texture won't render correctly.");
            }
    ...
    public ByteBuffer getPVRTextureBuffer() throws IOException {
        ...
        try {
            ByteBufferOutputStream os = new ByteBufferOutputStream(1024, 524288);
            StreamUtils.copy(inputStream, (OutputStream) os);
