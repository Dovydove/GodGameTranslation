...
public class ConfigChooser implements EGLConfigChooser {
    ...
    private EGLConfig chooseConfig(EGL10 pEGL, EGLDisplay pEGLDisplay, ConfigChooserMatcher pConfigChooserMatcher) throws IllegalArgumentException {
        ...
        if (this.mMultiSamplingRequested) {
            int eglConfigCount = getEGLConfigCount(pEGL, pEGLDisplay, EGLCONFIG_ATTRIBUTES_MULTISAMPLE);
            if (eglConfigCount > 0) {
            ...
            int eglConfigCount2 = getEGLConfigCount(pEGL, pEGLDisplay, EGLCONFIG_ATTRIBUTES_COVERAGEMULTISAMPLE_NVIDIA);
            if (eglConfigCount2 > 0) {
                this.mCoverageMultiSampling = true;
                return findEGLConfig(pEGL, pEGLDisplay, EGLCONFIG_ATTRIBUTES_COVERAGEMULTISAMPLE_NVIDIA, eglConfigCount2, pConfigChooserMatcher);
            }
        }
        int eglConfigCount3 = getEGLConfigCount(pEGL, pEGLDisplay, EGLCONFIG_ATTRIBUTES_FALLBACK);
        if (eglConfigCount3 > 0) {
            return findEGLConfig(pEGL, pEGLDisplay, EGLCONFIG_ATTRIBUTES_FALLBACK, eglConfigCount3, pConfigChooserMatcher);
        }
