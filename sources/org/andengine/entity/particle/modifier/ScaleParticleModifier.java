...
public class ScaleParticleModifier<T extends IEntity> extends BaseDoubleValueSpanParticleModifier<T> {
    ...
    public void onSetInitialValues(Particle<T> pParticle, float pScaleX, float pScaleY) {
        pParticle.getEntity().setScale(pScaleX, pScaleY);
    }

    /* access modifiers changed from: protected */
    public void onSetValues(Particle<T> pParticle, float pPercentageDone, float pScaleX, float pScaleY) {
        pParticle.getEntity().setScale(pScaleX, pScaleY);
    }
}
