...
public class ColorParticleModifier<T extends IEntity> extends BaseTripleValueSpanParticleModifier<T> {
    ...
    public void onSetInitialValues(Particle<T> pParticle, float pRed, float pGreen, float pBlue) {
        pParticle.getEntity().setColor(pRed, pGreen, pBlue);
    }

    /* access modifiers changed from: protected */
    public void onSetValues(Particle<T> pParticle, float pPercentageDone, float pRed, float pGreen, float pBlue) {
        pParticle.getEntity().setColor(pRed, pGreen, pBlue);
    }
}
